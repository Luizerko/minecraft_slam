...
Usually, the estimated 3D coordinates of the tracked points and camera positions are simultaneously optimized using bundle adjustment to minimize the reprojection error. But because we extracted the camera coordinates from the game parameters for each frame, meaning they are not estimated, we can only further improve our 3D reconstruction by refining the estimated 3D point coordinates. We do that by using structure-only bundle adjustment. 

To find the image coordinates of the estimated points we use the constructed projection matrices for each frame which transforms the 3D world coordinates of the given point into its corresponding image pixel coordinates. The pixel coordinates of each estimated point are stored in matrices Us (width coordinate) and Vs (height coordinate), each of dimensions (I, J) where I - index of the image frame, J - index of the point. Each point in the given matrix then represents the horizontal/vertical pixel cooridnate of point j on the image frame i. However, there are points that are visible on some frames and not visible on others. To account for them, we also construct the visibility matrix, also of size (I, J), where a value of 1 in the position (i, j) means that point j is visible in frame i, and 0 means that the point is not visible in the given frame. 

Now, having the projection matrices for each frame and the estimated 3D point coordinates of each point, we can project the given points to the pixel coordinates of the frames and minimize the difference with the true pixel coordinates of the given points. The true pixel coordinates of each point in the given frame is known from the track list constructed during the Tracking stage.

Since the projection matrices are of dimension (3, 4) we transform the estimated 3D point coordinates of dimension (3, 1) to homogenious form (4, 1) and project these coordinates using the projection matrix to image frame. This results in a vector of dimensions (3, 1), which needs to be normalized for the z coordinate (depth) to obtain the pixel coordinates for each point on each frame [x/z, y/z, 1] = [u, v, 1]. Having obtained the pixel coordinates for each point j on each image frame i (if the point is not visible simply use 0), we can use least squares optimization to adjust the estimated 3D coordinates, such that the error between their projection and the true pixel coordinates is minimized.